...
- **分布式追踪**：OpenTelemetry 集成用于跨服务的端到端请求追踪
- **指标收集**：Prometheus 兼容的指标用于监控 API 性能、数据库查询和任务处理
- **日志聚合**：Loki 用于跨所有服务的集中式日志收集和查询
- **可视化**：Grafana 仪表盘用于实时监控系统健康状况和性能
- **追踪可视化**：Tempo 用于分布式追踪可视化和性能瓶颈识别
- **数据转发**：Alloy 作为灵活的代理用于收集和转发可观测数据
- **Langfuse 追踪**：Alloy 可将 Agent 的 OTel Trace 统一转发到 Langfuse
