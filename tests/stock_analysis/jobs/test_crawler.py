...
async def test_stock_not_found(
    async_session: AsyncSession, cninfo_adapter: CNInfoAdapter, logger: logging.Logger
) -> None:
    payload: JobPayload = JobPayload(stock_code="999999")
    with pytest.raises(CrawlerError, match="not found"):
        await _crawl_cninfo_stock_data(async_session, payload, cninfo_adapter, logger)
