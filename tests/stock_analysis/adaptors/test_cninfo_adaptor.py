...
from stock_analysis.adaptors.cninfo import CNInfoAdaptor, CNInfoError
...
def test_adaptor_initialization(
    ...
    assert adaptor.timeout == timeout
    assert adaptor.limiter is async_limiter
    assert adaptor.retry_attempts == retry_attempts
    assert adaptor.available_endpoints == {"balance_sheets", "income_statement"}


def test_adaptor_no_specs_error(tmp_path: Path) -> None:
    empty_dir: Path = tmp_path / "empty"
    empty_dir.mkdir()

    with pytest.raises(CNInfoError) as e:
        CNInfoAdaptor(config_dir=empty_dir)
    assert "No valid CNInfo specs found" in str(e.value)


def test_get_spec(yaml_config_dir: Path) -> None:
    adaptor = CNInfoAdaptor(config_dir=yaml_config_dir)
    spec: ApiSpec = adaptor.get_spec("balance_sheets")
...
def test_get_spec_unknown(yaml_config_dir: Path) -> None:
    ...
    with pytest.raises(CNInfoError) as e:
        adaptor.get_spec(endpoint)
    assert f"Unknown CNInfo endpoint id: {endpoint}" in str(e.value)


def test_set_unfixed_request_params(yaml_config_dir: Path) -> None:
    adaptor = CNInfoAdaptor(config_dir=yaml_config_dir)
    params: tuple[RequestParam, ...] = adaptor.set_unfixed_request_params(
        "balance_sheets"
    )
    assert params == ()

    params = adaptor.set_unfixed_request_params("income_statement", stock_code="000001")
...
def test_set_unfixed_request_params_invalid(yaml_config_dir: Path) -> None:
    ...
    with pytest.raises(CNInfoError) as e:
        adaptor.set_unfixed_request_params("income_statement", stock_code=1)
    assert (
...
async def test_fetch_success(
    ...
    result: CNInfoFetchResult = await cninfo_adaptor.fetch("balance_sheets")
...
async def test_fetchclient_not_initialized(yaml_config_dir: Path) -> None:
    ...
    with pytest.raises(CNInfoError) as e:
        await adaptor.fetch("balance_sheets")
...
async def test_fetch_error(
    ...
    cninfo_adaptor.client = client_unauthorized[0]
    with pytest.raises(CNInfoError) as e:
        await cninfo_adaptor.fetch("income_statement")
    assert "Missing required params" in str(e.value)

    with pytest.raises(CNInfoError) as e:
        await cninfo_adaptor.fetch("income_statement", stock_code="000001")
...
async def test_fetch_retry(
    ...
    cninfo_adaptor.client = client_too_many_requests[0]
    with pytest.raises(CNInfoError) as e:
        await cninfo_adaptor.fetch("balance_sheets")
    assert "CNInfo HTTP" in str(e.value)
    assert client_too_many_requests[1]["count"] == cninfo_adaptor.retry_attempts
