<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : "Failed to load stock details";
    } finally {
      loading = false;
    }
  }
                ...
                <tbody>
                  {#each analysisDetails.data.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()) as analysis}
                    <tr>
                      ...
                      <td>
                        <span class="badge {analysis.score >= 70 ? 'badge-success' : analysis.score >= 40 ? 'badge-warning' : 'badge-danger'}">
                          {analysis.score.toFixed(2)}
                        </span>
                      </td>
                      <td>{Object.keys(analysis.metrics).length}</td>
                    </tr>
                  {/each}
                </tbody>
              </table>
        ...
        <div class="data-list">
          {#each stockDetails.cninfoData as item}
            <details class="card data-item">
              <summary>
                <span class="endpoint">{formatEndpoint(item.endpoint)}</span>
                <span
                  class="badge"
                  class:badge-success={item.responseCode === 200}
                  class:badge-danger={item.responseCode !== 200}
                >
                  {item.responseCode}
                </span>
      ...
      {:else}
        <div class="data-list">
          {#each stockDetails.yahooData as item}
            <details class="card data-item">
              <summary>
                <span class="endpoint">Historical Price Data</span>
                <span class="updated">{formatDate(item.updatedAt)}</span>
              </summary>
              <div class="data-content">
                <h4>{$t("stock.requestParams")}</h4>
