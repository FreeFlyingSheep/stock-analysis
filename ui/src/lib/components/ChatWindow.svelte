...
<div class={`chat-window ${floating ? "floating" : ""}`}>
  <header class="chat-header">
    ...
    {#if onClose}
      <button class="icon-button" onclick={onClose} aria-label={$t("chat.close")}>
        ✕
      </button>
    {/if}
  </header>
  ...
  <div class="input-row">
    {#if messages.length > 0}
      <button class="icon-button" onclick={clearChat} title={$t("chat.clear")} aria-label={$t("chat.clear")}>
        🗑️
      </button>
    {/if}
    <textarea
      ...
      disabled={isLoading}
    ></textarea>
    <button class="primary" onclick={sendMessage} disabled={!inputMessage.trim() || isLoading}>
      {$t("chat.send")}
    </button>
  </div>
